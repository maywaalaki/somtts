...
async def generate_chunk_audio(text, voice, rate, pitch, filename):
...
def handle_text(message):
    ...
    try:
        ...
        processed_text = process_text_with_groq(raw_text)
        
        text_chunks = chunk_text(processed_text, 300)
        ...
        for i, chunk in enumerate(text_chunks):
            chunk_file = os.path.join(DOWNLOADS_DIR, f"chunk_{user_id}_{i}_{int(time.time()*1000)}.mp3")
            loop.run_until_complete(generate_chunk_audio(chunk, voice_name, rate, pitch, chunk_file))
        ...
        with open(final_filename, "rb") as audio:
            bot.send_audio(
                ...
                audio,
                reply_to_message_id=message.message_id,
                title=f"Codka_{user_id}",
                performer="SomTTS Bot"
            )

    except Exception as e:
        bot.send_message(message.chat.id, f"Khalad ayaa dhacay: {e}", reply_to_message_id=message.message_id)
    finally:
        stop_event.set()
        for f in temp_files:
            if os.path.exists(f):
...
if __name__ == "__main__":
    if WEBHOOK_URL:
        ...
        print("Webhook setup skipped, running local.")
