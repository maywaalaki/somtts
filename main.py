...
def ensure_user_doc(user_id):
    ...
    if not doc:
        users_col.update_one({"user_id": uid}, {"$set": {"voice": "so-SO-MuuseNeural", "label": "Muuse 👨🏻‍🦱", "rate": 0, "pitch": 0, "welcomed": False}}, upsert=True)
        doc = users_col.find_one({"user_id": uid})
    else:
        if "welcomed" not in doc:
            users_col.update_one({"user_id": uid}, {"$set": {"welcomed": False}})
            doc = users_col.find_one({"user_id": uid})
    return doc
...
def start(message):
    ...
    ensure_user_doc(user_id_str)
    bot.send_message(
        message.chat.id,
        "Soo dhawow! Waxaan ahay Somali Text to Speech bot waxaana i sameeyay zack3d:\n\nii soo dir qoraal si aan ugu badalo cod💗",
...
def set_voice(message):
    ...
    choice = message.text
    if "Ubax" in choice:
        users_col.update_one({"user_id": user_id_str}, {"$set": {"voice": "so-SO-UbaxNeural", "label": "Ubax 👩🏻‍🦳"}}, upsert=True)
    elif "Muuse" in choice:
        users_col.update_one({"user_id": user_id_str}, {"$set": {"voice": "so-SO-MuuseNeural", "label": "Muuse 👨🏻‍🦱"}}, upsert=True)
    bot.send_message(
        message.chat.id,
...
def slider_handler(call):
    ...
    if call.data.startswith("rate_"):
        ...
        users_col.update_one({"user_id": uid}, {"$set": {"rate": val}}, upsert=True)
        try:
        ...
        if call.data == "pitch_up":
            ...
            val -= 5
        val = max(-100, min(100, val))
        users_col.update_one({"user_id": uid}, {"$set": {"pitch": val}}, upsert=True)
        try:
...
def handle_text(message):
    user_id_str = str(message.from_user.id)
    doc = ensure_user_doc(user_id_str)
    if not doc.get("welcomed", False):
        guide_text = (
            "⚠️ Fadlan raac tilmaamahan si aad u hesho cod tayo fiican leh:\n\n"
        ...
        sent = bot.send_message(message.chat.id, guide_text)
        try:
            bot.pin_chat_message(message.chat.id, sent.message_id, disable_notification=True)
        except Exception:
            pass
        users_col.update_one({"user_id": user_id_str}, {"$set": {"welcomed": True}}, upsert=True)
    admin_info = (
    ...
    voice_name = doc.get("voice", "so-SO-MuuseNeural")
    pitch_val = doc.get("pitch", 0)
    rate_val = doc.get("rate", 0)
