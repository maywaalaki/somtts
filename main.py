...
def help_command(message):
    if is_banned(message.from_user.id):
        ...
        return
    msg = bot.send_message(message.chat.id, "Fadlan soo qor dhibaatada aad wajahayso si aan kuu caawino:")
    bot.register_next_step_handler(msg, forward_help_to_admin)

def forward_help_to_admin(message):
    user_info = f"🆘 **CABASHO CUSUB**\n\n👤 Magaca: {message.from_user.first_name}\n🆔 ID: {message.from_user.id}\n👤 Username: @{message.from_user.username if message.from_user.username else 'Ma leh'}\n\n📝 **Dhibka jira:**\n{message.text}"
    try:
        bot.send_message(ADMIN_ID, user_info)
        bot.send_message(message.chat.id, "Mahadsanid! fariintaada waa loo gudbiyay Admin-ka.")
    except Exception as e:
        bot.send_message(message.chat.id, "Raaligo dhib ayaa ka dhacay gudbinta fariinta.")
...
def handle_text(message):
    ...
    text = process_text_with_groq(message.text)
