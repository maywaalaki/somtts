...
def _start_tts_loop(loop):
    asyncio.set_event_loop(loop)
    loop.run_forever()
threading.Thread(target=_start_tts_loop, args=(tts_loop,), daemon=True).start()

def _shutdown_loop():
    try:
        tts_loop.call_soon_threadsafe(tts_loop.stop)
    except Exception:
        pass
...
def handle_text(message):
    ...
    try:
        future = asyncio.run_coroutine_threadsafe(make_tts(), tts_loop)
        future.result()
