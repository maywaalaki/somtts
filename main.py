...
def _start_loop(loop):
    ...
    loop.run_forever()
loop_thread = threading.Thread(target=_start_loop, args=(global_loop,), daemon=True)
loop_thread.start()

async def _save_tts_async(text, voice_name, rate, pitch, filename):
...
def trim_leading_silence(sound, silence_thresh=-40.0, chunk_size=10):
    trim_ms = 0
    assert chunk_size > 0
...
def handle_text(message):
    ...
    try:
        for idx, chunk in enumerate(chunks):
            part_path = final_filename + f".part{idx}.mp3"
            fut = asyncio.run_coroutine_threadsafe(_save_tts_async(chunk, voice_name, rate, pitch, part_path), global_loop)
            try:
                ...
                bot.send_message(message.chat.id, f"Khalad intii la sameynayay TTS: {e}", reply_to_message_id=message.message_id)
                stop_event.set()
                return

        segments = []
        for p in part_files:
            sound = AudioSegment.from_file(p, format="mp3")
            trimmed = trim_leading_silence(sound, silence_thresh=-45.0, chunk_size=10)
            segments.append(trimmed)
        if not segments:
            bot.send_message(message.chat.id, "Khalad: wax segment ah kuma jiraan", reply_to_message_id=message.message_id)
            stop_event.set()
            return

        final_audio = segments[0]
        for seg in segments[1:]:
            final_audio = final_audio.append(seg, crossfade=10)

        final_audio.export(final_filename, format="mp3", bitrate="192k")
