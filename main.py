...
def get_user_data(user_id):
    ...
    if not user:
        ...
        if "pinned_sent" not in user:
            users_col.update_one({"user_id": str(user_id)}, {"$set": {"pinned_sent": False}}, upsert=True)
            user["pinned_sent"] = False
    return user
...
def forward_pinned_to_user(dest_chat_id, user_id, mark_sent=True):
    if not PIN_SOURCE:
        return False
    try:
        ...
        if pinned and getattr(pinned, "message_id", None):
            ...
            if mark_sent:
                users_col.update_one({"user_id": str(user_id)}, {"$set": {"pinned_sent": True}}, upsert=True)
            return True
        else:
            try:
                full = bot.get_chat(PIN_SOURCE)
                pinned = getattr(full, "pinned_message", None)
                if pinned and getattr(pinned, "message_id", None):
                    bot.forward_message(dest_chat_id, PIN_SOURCE, pinned.message_id)
                    if mark_sent:
                        users_col.update_one({"user_id": str(user_id)}, {"$set": {"pinned_sent": True}}, upsert=True)
...
def start(message):
    try:
        forward_pinned_to_user(message.chat.id, message.from_user.id, mark_sent=False)
    except:
        pass
...
def cmd_cod(message):
    kb = InlineKeyboardMarkup()
    kb.row(InlineKeyboardButton("Ubax", callback_data="setvoice_Ubax"), InlineKeyboardButton("Muuse", callback_data="setvoice_Muuse"))
    kb.add(InlineKeyboardButton("Codka trnd ga ah🔥", callback_data="setvoice_Trend"))
    bot.send_message(message.chat.id, "Dooro codka aad rabto:", reply_markup=kb, reply_to_message_id=message.message_id)

@bot.message_handler(func=lambda m: m.text in ["Ubax", "Muuse", "Ubax 👩🏻‍🦳", "Muuse 👨🏻‍🦱"])
def set_voice(message):
    ...
    bot.send_message(
        message.chat.id,
        f"Ok hada Codka waa: {voice_data['label']}\n\n ii soo dir qoraalka 👍",
        reply_to_message_id=message.message_id
    )
...
@bot.callback_query_handler(func=lambda call: call.data and call.data.split('_')[0] in ("rate", "pitch", "setvoice"))
    ...
    try:
        if call.data.startswith("rate_"):
            ...
            try:
                ...
                pass
        elif call.data.startswith("setvoice_"):
            ...
            if target == "Ubax":
                ...
                voice_data = {"name": "so-SO-MuuseNeural", "label": "Codka trnd ga ah🔥"}
                updates = {"voice": voice_data, "pitch": 45, "rate": 30}
            else:
                voice_data = user.get("voice", {"name": "so-SO-MuuseNeural", "label": "Muuse"})
                updates = {"voice": voice_data}
            update_user_data(uid, updates)
            try:
                bot.edit_message_text(f"Ok hada Codka waa: {updates['voice']['label']}\n\n ii soo dir qoraalka 👍", chat_id=call.message.chat.id, message_id=call.message.message_id)
            except Exception:
                pass
    except Exception:
        pass
...
def handle_text(message):
    ...
    if not user.get("pinned_sent", False):
        try:
            forward_pinned_to_user(message.chat.id, user_id, mark_sent=True)
            user = get_user_data(user_id)
        except:
            pass
    admin_info = (
