    ...
    def process_and_send():
        ...
        async def make_tts():
            ...
            if len(text) > 250:
                ...
                current_chunk = ""
                for word in words:
                    if len(current_chunk) + len(word) + 1 < 250:
                        current_chunk += " " + word
                    else:
            ...
            if len(chunks) == 1:
                tts = edge_tts.Communicate(chunks[0], voice_name, rate=rate, pitch=pitch)
                await tts.save(final_filename)
            else:
                temp_files = []
                base_name = str(int(time.time()*1000))
                for i, chunk in enumerate(chunks):
                    temp_file = os.path.join(DOWNLOADS_DIR, f"part_{user_id}_{base_name}_{i}.mp3")
                    tts = edge_tts.Communicate(chunk, voice_name, rate=rate, pitch=pitch)
                    await tts.save(temp_file)
                    temp_files.append(temp_file)
                
                list_file_path = os.path.join(DOWNLOADS_DIR, f"list_{user_id}_{base_name}.txt")
                ...
                subprocess.run([
                    ...
                    "-i", list_file_path, "-c", "copy", final_filename
