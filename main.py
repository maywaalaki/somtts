...
def cod_command(message):
    kb = InlineKeyboardMarkup()
    kb.row(InlineKeyboardButton("Codka qulaa sada🔥", callback_data="voice:Codka qulaa sada🔥"), InlineKeyboardButton("Ubax", callback_data="voice:Ubax"), InlineKeyboardButton("Muuse", callback_data="voice:Muuse"))
    bot.send_message(message.chat.id, "Dooro codka adigoo gujinaya:", reply_markup=kb, reply_to_message_id=message.message_id)
...
@bot.callback_query_handler(func=lambda call: call.data and (call.data.startswith(("rate_", "pitch_")) or call.data.startswith("voice:")))
def callback_handler(call):
    ...
    if call.data.startswith("rate_"):
        ...
        if "Ubax" in choice:
            ...
            update_user_data(uid, {"voice": voice_data})
        elif "qulaa" in choice or "Codka qulaa" in choice:
            voice_data = {"name": "so-SO-MuuseNeural", "label": "Codka qulaa sada🔥"}
            update_user_data(uid, {"voice": voice_data, "pitch": 45, "rate": 30})
        else:
            voice_data = {"name": "so-SO-MuuseNeural", "label": "Muuse"}
            update_user_data(uid, {"voice": voice_data})
        try:
            bot.answer_callback_query(call.id, text=f"Codka waa: {choice}")
        except Exception:
            pass
        try:
            bot.send_message(call.message.chat.id, f"Ok hada Codka waa: {choice}\n\n ii soo dir qoraalka 👍")
        except Exception:
            pass
    try:
